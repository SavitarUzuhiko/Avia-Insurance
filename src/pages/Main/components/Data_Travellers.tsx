...
export const DataTravellers = ({ num, index, isBuyer, remove }: Props) => {
  const {
    register,
    control,
    setValue,
  ...
  const getTravellerError = (field: keyof Traveller) =>
    (errors.travellers as FieldErrors<Traveller>[] | undefined)?.[index]?.[field]?.message;
  ...
  return (
    <div className="shadow-[0_2px_5px_3px_rgba(0,0,0,0.2)] rounded-lg p-4">
      ...
      <div className="space-y-4">
        {/* First name */}
        <div>
          <Label htmlFor={`travellers.${index}.firstName`} className="text-md">
            First name
          </Label>
          <Input
          ...
          />
          {getTravellerError('firstName') && (
            <p className="text-red-500 text-sm">{getTravellerError('firstName')}</p>
          )}
        </div>
        ...
        <div>
          ...
          />
          {getTravellerError('lastName') && (
            <p className="text-red-500 text-sm">{getTravellerError('lastName')}</p>
          )}
        </div>
